...
Setup:
1) pip install python-telegram-bot pymongo --upgrade
...
MONGO_URI = os.environ.get("MONGO_URI", "mongodb+srv://sahilx786tg8_db_user:Z4CKCy5f6VIayBNJ@cluster0.cmhn6cq.mongodb.net/SmmPanel?appName=Cluster0")
_mongo_client = None
_mongo_db = None
...
def _get_db():
    ...
    if _mongo_db is None:
        ...
        _mongo_db = _mongo_client["SmmPanel"]
    return _mongo_db


DATA_DIR_WRITABLE = True  # MongoDB is always writable


def _ensure_data_dir():
    pass  # MongoDB — no directory needed


def load_data():
    """Load all bot data from MongoDB."""
    try:
        db = _get_db()
        doc = db["botdata"].find_one({"_id": "main"})
        if doc:
            doc.pop("_id", None)
            stored_version = doc.get("_version", 1)
            if stored_version < DATA_FILE_VERSION:
                print(f"📊 Upgrading data: v{stored_version} → v{DATA_FILE_VERSION}")
                doc = migrate_data_v1_to_v2(doc)
                doc["_version"] = DATA_FILE_VERSION
                save_data(doc)
            doc.setdefault("processed_tx", [])
            doc.setdefault("custom_services", [])
            doc.setdefault("custom_texts", {})
            doc.setdefault("pending_deposits", {})
            doc.setdefault("_version", DATA_FILE_VERSION)
            return doc
...
def save_data(data):
    """Save all bot data to MongoDB."""
    try:
        db = _get_db()
        save_doc = dict(data)
        save_doc["_id"] = "main"
        db["botdata"].replace_one({"_id": "main"}, save_doc, upsert=True)
    except Exception as e:
...
async def admin_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "admin_back":
        ...
        try:
            db = _get_db()
            db["botdata"].find_one({"_id": "main"})
            text = (
                "💾 *Storage Check*\n\n"
                "✅ *MongoDB Atlas* connected!\n"
                "Data is stored in the cloud — safe forever, no Railway Volume needed."
            )
        except Exception as e:
            text = f"💾 *Storage Check*\n\n❌ MongoDB connection failed!\n\n`{e}`"
        await query.edit_message_text(text=text, reply_markup=back_kb, parse_mode=ParseMode.MARKDOWN)
        return
...
async def _post_init_storage_check(app):
    """Runs once the bot starts. Confirms MongoDB is connected."""
    try:
        db = _get_db()
        db["botdata"].find_one({"_id": "main"})
        print("✅ MongoDB connected successfully!")
        try:
            await app.bot.send_message(chat_id=ADMIN_ID, text="✅ *Bot started!* MongoDB connected — data is safe.", parse_mode=ParseMode.MARKDOWN)
        except Exception:
            pass
    except Exception as e:
        warning = f"⚠️ *MongoDB connection FAILED!*\n\n`{e}`\n\nCheck your MONGO\_URI environment variable."
        print(warning)
        try:
            await app.bot.send_message(chat_id=ADMIN_ID, text=warning, parse_mode=ParseMode.MARKDOWN)
        except Exception as e2:
            print(f"⚠️ Couldn't send warning to admin: {e2}")
