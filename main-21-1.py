...
async def create_oxapay_invoice(usd_amount: float, order_id: str) -> dict:
    ...
    payload = {
        ...
        "callbackUrl": "",       # no server webhook needed — we poll
        "returnUrl": "",
        "description": f"SMM Panel deposit {order_id}",
        "orderId": order_id,
    }
    async with httpx.AsyncClient(timeout=20) as client:
...
async def verify_oxapay_deposit(user_id: int, track_id: str, usd_amount: float) -> tuple | None:
    ...
    Returns (usd_credited, token_amount, track_id) or None if not yet paid."""
    try:
    ...
    status = info.get("result", {}).get("status") if isinstance(info.get("result"), dict) else info.get("status")
    # Some OxaPay responses: top-level 'status' field
    if status is None:
        status = info.get("result")

    print(f"OxaPay status for {track_id}: {status} | raw: {info}")

    if status not in ("Paid", "Confirming"):
        return None

    # Only credit once (claim by trackId)
    if not _claim_tx_hash(track_id):
        return None  # already credited

    # Credit the exact USD amount the user requested
    ...
    if data == "services":
        ...
        if result:
            ...
            text = (
                ...
                "Please complete the payment via the link you received.\n"
                "OxaPay confirmation may take a minute — tap *Try Again* after paying."
            )
            if error_msg:
                text += f"\n\n⚠️ *Debug:* {error_msg}"
        await query.edit_message_text(text=text, reply_markup=kb, parse_mode=ParseMode.MARKDOWN)
...
async def deposit_amount_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        invoice = await create_oxapay_invoice(usd_amount, order_id)
    except Exception as e:
