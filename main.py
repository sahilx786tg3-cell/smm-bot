...
TONCENTER_API = "https://toncenter.com/api/v3/transactions"
...
TONCENTER_API_KEY = "3b00906489eaa7ef90be1325ead1ba4012e83b2630e193a7cb7060abacec1eca"
...
# ---- TON (native GRAM) — TonCenter v3 API ----
async def fetch_ton_native_transactions(limit: int = 100):
    """Fetches recent incoming transactions for the GRAM wallet from TonCenter v3.
    v3 is faster, indexed, and more reliable than v2. Returns list of tx dicts."""
    params = {"account": GRAM_WALLET_ADDRESS, "limit": limit}
    headers = {}
    ...
    for attempt in range(3):
        try:
            async with httpx.AsyncClient(timeout=15) as client:
                ...
                resp.raise_for_status()
                data = resp.json()
                # v3 returns {"transactions": [...]} directly — no "ok" wrapper
                txs = data.get("transactions", [])
                print(f"✅ TonCenter v3: fetched {len(txs)} transactions")
                return txs
        except Exception as e:
            last_error = e
            print(f"⚠️ TonCenter v3 attempt {attempt+1} failed: {e}")
            if attempt < 2:
                await asyncio.sleep(2)
    print(f"⚠️ TonCenter v3 fetch failed after retries: {last_error}")
    return []


def extract_ton_comment(in_msg: dict) -> str:
    """Extracts memo/comment from a v3 in_msg object."""
    if not in_msg:
        return ""
    # v3: decoded_body -> comment (most reliable)
    decoded = in_msg.get("decoded_body") or {}
    if decoded.get("comment"):
        return decoded["comment"].strip()
...
async def verify_ton_native_deposit(user_id: int, min_amount: float = 0.0, memo_code: str = None, method: dict = None):
    ...
    for tx in transactions:
        ...
        if value_nanotons <= 0:
            continue
        tx_hash = tx.get("hash", "")  # v3 API: hash is at top-level
        if not tx_hash:
            continue
