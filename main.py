...
def evm_unique_offset(user_id: int) -> float:
    """
    Polygon/BSC have no memo field, so we can't tag a transfer with the user's ID.
    Instead we add a tiny, near-unique fractional amount (worth a fraction of a cent)
    to what they're asked to send, and match deposits by that exact amount instead.
    """
    return (user_id % 9000 + 100) / 1_000_000  # e.g. 0.000100 .. 0.009099
...
async def verify_ton_native_deposit(user_id: int, min_amount: float = 0.0):
    """Checks for a new GRAM(TON) deposit whose memo == user_id. Returns (usd, token_amt, tx_hash) or None."""
    data = load_data()
    ...
    for tx in transactions:
        ...
        if extract_ton_comment(in_msg) != memo:
            continue
        ...
        _credit_deposit(user_id, usd_amount, tx_hash, "GRAM (TON) Autopay", token_amount)
...
async def verify_ton_usdt_deposit(user_id: int, min_amount: float = 0.0):
    """Checks for a new USDT(TON) deposit whose memo == user_id. Returns (usd, token_amt, tx_hash) or None."""
    data = load_data()
    processed = set(data.get("processed_tx", []))
    memo = str(user_id)
    try:
    ...
    for tr in transfers:
        ...
        _credit_deposit(user_id, usd_amount, tx_hash, "USDT (TON) Autopay", token_amount)
        return usd_amount, token_amount, tx_hash
    return None
...
async def verify_evm_deposit(method_key: str, user_id: int, expected_token_amount: float):
    ...
    to send (see evm_unique_offset). Returns (usd, token_amt, tx_hash) or None.
    """
    ...
    for tx in txs:
        ...
        usd_amount = round(token_amount * price, 3)
        _credit_deposit(user_id, usd_amount, tx_hash, f"{method['label']} Autopay", token_amount)
        return usd_amount, token_amount, tx_hash
    return None


def _credit_deposit(user_id: int, usd_amount: float, tx_hash: str, label: str, token_amount: float):
    """Shared helper: credits the user's wallet and records the transaction (marks tx as processed)."""
...
async def verify_deposit(method_key: str, user_id: int, expected_amount: float):
    """Dispatches verification to the right chain handler based on the chosen payment method."""
    method = PAYMENT_METHODS[method_key]
    if method_key == "ton_gram":
        return await verify_ton_native_deposit(user_id, min_amount=expected_amount)
    if method_key == "ton_usdt":
        return await verify_ton_usdt_deposit(user_id, min_amount=expected_amount)
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "services":
        ...
        try:
            result = await verify_deposit(pending["method_key"], user_id, pending["token"])
        except Exception:
            result = None
...
async def admin_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "admin_back":
        ...
        if last_tx:
            ...
            for t in last_tx:
                lines.append(f"🆔 `{t['user_id']}` | `${t['amount']:.2f}` | {t['time']}")
...
async def deposit_amount_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    offset = 0.0 if method["uses_memo"] else evm_unique_offset(user_id)
    ...
    if method["uses_memo"]:
        text = (
            ...
            f"`{user_id}`\n\n"
            "⚠️ Send the *exact amount* above with the memo included — sending less "
        ...
        text = (
            ...
            "(every decimal matters) so we can match it to your account. Sending a rounded "
            "amount won't be credited automatically.\n\n"
            "✅ After sending, tap *Verify Payment* below."
        )
    kb = InlineKeyboardMarkup([
