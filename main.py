...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "services":
        ...
        # 1-minute code expiry: an old, unsent code shouldn't stay valid forever and
        # risk colliding with confusion from a later deposit attempt. After expiry the
        # user must start a fresh deposit, which issues a brand-new memo code.
        if time.time() - pending.get("created_at", 0) > 60:
            context.user_data.pop("pending_deposit", None)
            clear_pending_deposit(user_id)
            text = (
                "⌛ *This deposit code has expired* (valid for 1 minute).\n\n"
                "Please start a new deposit to get a fresh memo code."
            )
...
async def deposit_amount_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        ...
        f"`{deposit_code}`\n\n"
        "⏱️ *This code is valid for 1 minute.* If it expires before you send, "
        "start a new deposit to get a fresh code.\n\n"
