...
async def create_oxapay_invoice(usd_amount: float, order_id: str, user_id: int = None) -> dict:
    ...
    payload = {
        ...
        "returnUrl": "",
        "description": f"Deposit by User {user_id}" if user_id else f"SMM Panel deposit {order_id}",
        "orderId": order_id,
    }
    async with httpx.AsyncClient(timeout=20) as client:
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "services":
        ...
        try:
            ...
            if oxapay_status in ("Paid", "Confirming"):
                if _claim_tx_hash(track_id):
                    _finalize_deposit(user_id, usd_amount, track_id, "OxaPay Autopay", usd_amount)
                    result = (usd_amount, usd_amount, track_id)
                else:
                    # Already claimed — check if balance was actually added
                    result = (usd_amount, usd_amount, track_id)
        except Exception as e:
            error_msg = str(e)
...
async def oxapay_webhook_handler(request: web.Request) -> web.Response:
    ...
    _finalize_deposit(matched_user_id, usd_amount, track_id, "OxaPay Autopay", usd_amount)
    clear_pending_deposit(matched_user_id)
    print(f"✅ Webhook: Credited ${usd_amount} to user {matched_user_id}")
