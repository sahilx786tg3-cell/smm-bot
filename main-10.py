...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
                    ...
                    try:
                        await context.bot.send_message(
                            ...
                            text=(
                                ...
                                f"💵 *+${bonus:.2f}* added to your balance.\n"
    ...
    if not user.get("welcome_bonus_given_notified"):
        ...
        update_user(user_id, user)
        welcome_text = "🎁 *Welcome Bonus:* `$0.01` has been added to your wallet — enjoy a free try!\n\n" + welcome_text
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "services":
        ...
        text = (
            "🤝 *Affiliate Program*\n\n"
            f"🎁 *${bonus:.2f} Signup Bonus* — Jab bhi koi aapke link se join kare, turant *${bonus:.2f}* milega!\n\n"
            f"💸 *{comm*100:.0f}% Commission* — Referrals ke har order ka *{comm*100:.0f}%* aapko milta hai — lifetime!\n\n"
            "No limits, no effort — just passive income! 🚀\n\n"
            f"🔗 *Aapka Affiliate Link:*\n`{link}`"
        )
        kb = InlineKeyboardMarkup([
        ...
        signup_earned = round(ref_count * bonus, 4)
        commission_earned = round(max(0, user["total_earned"] - signup_earned), 4)
        text = (
            "📊 *Affiliate Statistics*\n\n"
            f"👥 *Total Referred:* {ref_count}\n"
            f"🎁 *Signup Bonuses:* `${signup_earned:.2f}` ({ref_count} × ${bonus:.2f})\n"
            f"🛒 *Order Commission:* `${commission_earned:.2f}` ({comm*100:.0f}% on orders)\n"
