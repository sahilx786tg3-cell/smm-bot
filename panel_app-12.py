...
POLYGONSCAN_API_KEY = "NHV2XVJXMS3P89FZDG1TB5F9SSFCEVX2HM"
BSCSCAN_API_KEY = "NHV2XVJXMS3P89FZDG1TB5F9SSFCEVX2HM"
...
PAYMENT_METHODS = {
    "ton_gram": {
        ...
        "coingecko_id": "the-open-network", "address": GRAM_WALLET_ADDRESS, "uses_memo": True,
    },
    "ton_usdt": {
        "label": "💵 USDT (TON)", "symbol": "USDT", "chain": "ton", "asset": "jetton",
        "coingecko_id": "tether", "address": GRAM_WALLET_ADDRESS, "jetton_master": USDT_TON_MASTER,
        "uses_memo": True,
    },
    "pol_native": {
        "label": "🟣 POL (Polygon)", "symbol": "POL", "chain": "polygon", "asset": "native",
        "coingecko_id": "matic-network", "address": EVM_WALLET_ADDRESS, "decimals": 18, "uses_memo": False,
    },
    "pol_usdt": {
        "label": "💵 USDT (Polygon)", "symbol": "USDT", "chain": "polygon", "asset": "erc20",
        "coingecko_id": "tether", "address": EVM_WALLET_ADDRESS, "contract": USDT_POLYGON_CONTRACT,
        "decimals": 6, "uses_memo": False,
    },
    "bnb_native": {
        "label": "🟡 BNB (BSC)", "symbol": "BNB", "chain": "bsc", "asset": "native",
        "coingecko_id": "binancecoin", "address": EVM_WALLET_ADDRESS, "decimals": 18, "uses_memo": False,
    },
    "bnb_usdt": {
        "label": "💵 USDT (BEP20)", "symbol": "USDT", "chain": "bsc", "asset": "erc20",
        "coingecko_id": "tether", "address": EVM_WALLET_ADDRESS, "contract": USDT_BSC_CONTRACT,
...
async def get_token_price_usd(coingecko_id: str) -> float:
    """Fetches (and briefly caches) a token's USD price from CoinGecko.
    Retries a couple of times before giving up, since CoinGecko's free
    tier occasionally rate-limits or has a transient hiccup."""
    now = time.time()
    ...
    # All retries failed — serve a stale cached price if we have one,
    # rather than blocking the deposit entirely.
    if cached:
        return cached[0]
    raise last_error
...
async def verify_ton_native_deposit(user_id: int, min_amount: float = 0.0):
    ...
    for tx in transactions:
        ...
        if min_amount and token_amount + 1e-9 < min_amount:
            continue
        price = await get_token_price_usd("the-open-network")
...
async def fetch_evm_native_txs(api_url: str, api_key: str, address: str, limit: int = 50):
    async with httpx.AsyncClient(timeout=15) as client:
...
async def fetch_evm_token_txs(api_url: str, api_key: str, address: str, contract: str, limit: int = 50):
    async with httpx.AsyncClient(timeout=15) as client:
        resp = await client.get(api_url, params={
...
async def verify_evm_deposit(method_key: str, user_id: int, expected_token_amount: float):
    ...
    api_url = POLYGONSCAN_API_URL if method["chain"] == "polygon" else BSCSCAN_API_URL
    api_key = POLYGONSCAN_API_KEY if method["chain"] == "polygon" else BSCSCAN_API_KEY
    ...
    try:
        if method["asset"] == "native":
            txs = await fetch_evm_native_txs(api_url, api_key, method["address"])
        else:
            txs = await fetch_evm_token_txs(api_url, api_key, method["address"], method["contract"])
    except Exception:
        return None
    ...
    for tx in txs:
        ...
        price = await get_token_price_usd(method["coingecko_id"])
        usd_amount = round(token_amount * price, 3)
