...
def load_data():
    ...
    return {"users": {}, "transactions": [], "processed_tx": [], "custom_services": [], "custom_texts": {}}
...
def generate_unique_evm_offset() -> float:
    """Generates a fresh random micro-offset for a Polygon/BSC deposit and permanently
    reserves it (persisted to disk) so it can never be issued again — not even to the
    same user on a later deposit. Replaces the old scheme where the offset was derived
    purely from the user_id and so stayed identical across every deposit session,
    meaning an old already-matched transaction's amount could in theory satisfy a
    brand-new deposit request."""
    with _DATA_LOCK:
        data = load_data()
        used = set(data.setdefault("used_evm_offsets", []))
        while True:
            raw = secrets.randbelow(899900) + 100  # 100 .. 900000 (six-decimal range)
            key = str(raw)
            if key in used:
                continue
            used.add(key)
            data["used_evm_offsets"] = list(used)
            save_data(data)
            return raw / 1_000_000  # e.g. 0.000100 .. 0.900000
...
async def verify_ton_native_deposit(user_id: int, min_amount: float = 0.0, memo_code: str = None):
    ...
    for tx in transactions:
        ...
        if min_amount and token_amount + 1e-9 < min_amount:
            continue
        # Claim the hash atomically, BEFORE the price-fetch await below. This closes
        # the race window that let a tx get double-credited if the user tapped
        # "Try Again" while a previous check was still awaiting the price.
        if not _claim_tx_hash(tx_hash):
            continue
        price = await get_token_price_usd(PAYMENT_METHODS["ton_gram"])
        usd_amount = round(token_amount * price, 3)
...
async def verify_evm_deposit(method_key: str, user_id: int, expected_token_amount: float):
    """
...
async def verify_deposit(method_key: str, user_id: int, expected_amount: float, memo_code: str = None):
    """Dispatches verification to the right chain handler based on the chosen payment method."""
    method = PAYMENT_METHODS[method_key]
    if method_key == "ton_gram":
        return await verify_ton_native_deposit(user_id, min_amount=expected_amount, memo_code=memo_code)
    if method_key == "ton_usdt":
        return await verify_ton_usdt_deposit(user_id, min_amount=expected_amount, memo_code=memo_code)
    return await verify_evm_deposit(method_key, user_id, expected_amount)
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "services":
        ...
        try:
            result = await verify_deposit(pending["method_key"], user_id, pending["token"], pending.get("code"))
        except Exception:
            result = None
