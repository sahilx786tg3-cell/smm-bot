...
def get_explorer_link(chain: str, tx_hash: str) -> str:
    """Builds a clickable block-explorer URL for the given chain + tx hash."""
    if chain == "ton":
...
def guess_chain_from_label(label: str) -> str:
    """Old transaction records only stored a text label (e.g. 'GRAM (TON) Autopay'),
    not a chain key — this infers the chain so we can still build an explorer link."""
    label = (label or "").upper()
    if "TON" in label:
        return "ton"
    if "POLYGON" in label or "POL" in label:
        return "polygon"
    if "BSC" in label or "BEP20" in label or "BNB" in label:
        return "bsc"
    if "ETHEREUM" in label or "ETH" in label:
        return "ethereum"
    return "ton"  # default fallback
...
# ---- Multi-chain Autopay config ----
GRAM_WALLET_ADDRESS = "UQB4rr2U6zTQCNq-jENXpmfQjUCe6cSpiy2tfSeT9JNyo3Rj"   # TON wallet (GRAM + USDT-TON)
EVM_WALLET_ADDRESS = "0xaA0d8f4FDd90280A1c710BB730247650FBB56ED7"          # Polygon + BSC wallet (POL/BNB/USDT)

USDT_TON_MASTER = "EQCxE6mUtQJKFnGfaROTKOt1lZbDiiX1kCixRv7Nw2Id_sDs"  # official USDT jetton master on TON
USDT_POLYGON_CONTRACT = "0xc2132D05D31c914a87C6611C10748AEb04B58e8F"  # official USDT contract on Polygon
USDT_BSC_CONTRACT = "0x55d398326f99059fF775485246999027B3197955"     # official USDT (BEP20) contract on BSC
USDT_ETH_CONTRACT = "0xdAC17F958D2ee523a2206206994597C13D831ec7"     # official USDT contract on Ethereum
...
CHAIN_IDS = {"polygon": 137, "bsc": 56, "ethereum": 1}
COINGECKO_API = "https://api.coingecko.com/api/v3/simple/price"

# Free keys: polygonscan.com/myapikey and bscscan.com/myapikey — required for Polygon/BSC autopay to work.
ETHERSCAN_API_KEY = "NHV2XVJXMS3P89FZDG1TB5F9SSFCEVX2HM"  # Etherscan V2 unified key — works for ETH, Polygon, BSC, etc.

MIN_DEPOSIT_USD = 0.1

# Each payment method users can pick under Deposit.
# binance_symbol = primary price source (Binance public API, no key needed, very reliable).
# coingecko_id = fallback price source if Binance is unreachable.
PAYMENT_METHODS = {
    "ton_gram": {
        "label": "💎 GRAM (TON)", "symbol": "TON", "chain": "ton", "asset": "native",
        "binance_symbol": "TONUSDT", "coingecko_id": "the-open-network",
        "address": GRAM_WALLET_ADDRESS, "uses_memo": True,
    },
    "ton_usdt": {
        "label": "💵 USDT (TON)", "symbol": "USDT", "chain": "ton", "asset": "jetton",
        "binance_symbol": None, "coingecko_id": "tether",
        "address": GRAM_WALLET_ADDRESS, "jetton_master": USDT_TON_MASTER,
        "uses_memo": True,
    },
    "pol_native": {
        "label": "🟣 POL (Polygon)", "symbol": "POL", "chain": "polygon", "asset": "native",
        "binance_symbol": "POLUSDT", "coingecko_id": "matic-network",
        "address": EVM_WALLET_ADDRESS, "decimals": 18, "uses_memo": False,
    },
    "pol_usdt": {
        "label": "💵 USDT (Polygon)", "symbol": "USDT", "chain": "polygon", "asset": "erc20",
        "binance_symbol": None, "coingecko_id": "tether",
        "address": EVM_WALLET_ADDRESS, "contract": USDT_POLYGON_CONTRACT,
        "decimals": 6, "uses_memo": False,
    },
    "bnb_native": {
        "label": "🟡 BNB (BSC)", "symbol": "BNB", "chain": "bsc", "asset": "native",
        "binance_symbol": "BNBUSDT", "coingecko_id": "binancecoin",
        "address": EVM_WALLET_ADDRESS, "decimals": 18, "uses_memo": False,
    },
    "bnb_usdt": {
        "label": "💵 USDT (BEP20)", "symbol": "USDT", "chain": "bsc", "asset": "erc20",
        "binance_symbol": None, "coingecko_id": "tether",
        "address": EVM_WALLET_ADDRESS, "contract": USDT_BSC_CONTRACT,
        "decimals": 18, "uses_memo": False,
    },
    "eth_usdt": {
        "label": "💵 USDT (Ethereum)", "symbol": "USDT", "chain": "ethereum", "asset": "erc20",
        "binance_symbol": None, "coingecko_id": "tether",
        "address": EVM_WALLET_ADDRESS, "contract": USDT_ETH_CONTRACT,
        "decimals": 6, "uses_memo": False,
    },
}
...
def generate_unique_evm_offset() -> float:
    """Generates a fresh random micro-offset for THIS Polygon/BSC deposit, so the exact
    amount the user is told to send differs every time instead of being the same fixed
    offset every time (the old behaviour, derived purely from user_id).

    Note: this offset is only a *search hint* to quickly find the right transaction —
    the actual security boundary against double-crediting is _claim_tx_hash(), which
    guarantees every on-chain transaction hash can be credited at most once, no matter
    what amount it was for. Because of that, this offset does NOT need to be reserved
    forever: USDT only has 6 on-chain decimals, so a permanently-growing reservation
    list would eventually exhaust the (small) pool of genuinely sub-cent values and the
    bot would hang trying to find a free one. A fresh random pick each time is enough.
    """
    raw = secrets.randbelow(8900) + 100  # 100 .. 8999
    return raw / 1_000_000  # 0.000100 .. 0.008999 (sub-cent, same magnitude as before)
...
# ---- TON (native GRAM) ----
async def fetch_ton_native_transactions(limit: int = 50):
...
async def verify_ton_native_deposit(user_id: int, min_amount: float = 0.0, memo_code: str = None, expected_usd: float = None):
    ...
    for tx in transactions:
        ...
        _finalize_deposit(user_id, usd_amount, tx_hash, "GRAM (TON) Autopay", token_amount)
        return usd_amount, token_amount, tx_hash
    return None


# ---- TON (USDT jetton) — via tonapi.io's jetton transfer history ----
async def fetch_ton_jetton_transfers(limit: int = 50):
    async with httpx.AsyncClient(timeout=15) as client:
        resp = await client.get(
            f"{TONAPI_URL}/accounts/{GRAM_WALLET_ADDRESS}/jettons/{USDT_TON_MASTER}/history",
            params={"limit": limit},
        )
        resp.raise_for_status()
        resp_json = resp.json()
        return resp_json.get("events", []) or resp_json.get("transfers", []) or []


async def verify_ton_usdt_deposit(user_id: int, min_amount: float = 0.0, memo_code: str = None):
    """Checks for a new USDT(TON) deposit whose memo matches the one-time code issued
    for THIS specific deposit request. Returns (usd, token_amt, tx_hash) or None."""
    if not memo_code:
        return None
    try:
        transfers = await fetch_ton_jetton_transfers()
    except Exception:
        return None

    for tr in transfers:
        comment = str(tr.get("comment") or tr.get("payload_text") or "").strip()
        if comment != memo_code:
            continue
        raw_amount = tr.get("amount") or tr.get("value") or 0
        tx_hash = tr.get("tx_hash") or tr.get("event_id") or tr.get("hash") or ""
        if not tx_hash:
            continue
        token_amount = float(raw_amount) / 1e6  # USDT jetton uses 6 decimals
        if token_amount <= 0:
            continue
        if min_amount and token_amount + 1e-6 < min_amount:
            continue
        if not _claim_tx_hash(tx_hash):
            continue
        usd_amount = round(token_amount, 3)  # USDT ≈ $1
        _finalize_deposit(user_id, usd_amount, tx_hash, "USDT (TON) Autopay", token_amount)
        return usd_amount, token_amount, tx_hash
    return None


# ---- Polygon / BSC (native + ERC20 USDT) via PolygonScan / BscScan ----
async def fetch_evm_native_txs(chain: str, api_key: str, address: str, limit: int = 50):
    async with httpx.AsyncClient(timeout=15) as client:
        resp = await client.get(ETHERSCAN_V2_API_URL, params={
            "chainid": CHAIN_IDS[chain],
            "module": "account", "action": "txlist", "address": address,
            "sort": "desc", "page": 1, "offset": limit, "apikey": api_key,
        })
        resp.raise_for_status()
        data = resp.json()
...
async def fetch_evm_token_txs(chain: str, api_key: str, address: str, contract: str, limit: int = 50):
    async with httpx.AsyncClient(timeout=15) as client:
        resp = await client.get(ETHERSCAN_V2_API_URL, params={
            "chainid": CHAIN_IDS[chain],
            "module": "account", "action": "tokentx", "address": address,
            "contractaddress": contract, "sort": "desc", "page": 1, "offset": limit, "apikey": api_key,
        })
        resp.raise_for_status()
        data = resp.json()
        return data.get("result", []) if data.get("status") == "1" else []
...
async def verify_evm_deposit(method_key: str, user_id: int, expected_token_amount: float, expected_usd: float = None):
    """
    Polygon/BSC have no memo, so we match by the exact unique amount the user was told
    to send (see generate_unique_evm_offset). Returns (usd, token_amt, tx_hash) or None.
    """
    method = PAYMENT_METHODS[method_key]
    chain = method["chain"]
    api_key = ETHERSCAN_API_KEY  # same Etherscan V2 key works for all chains
    decimals = method["decimals"]

    try:
        if method["asset"] == "native":
            txs = await fetch_evm_native_txs(chain, api_key, method["address"])
        else:
            txs = await fetch_evm_token_txs(chain, api_key, method["address"], method["contract"])
    except Exception:
        return None

    for tx in txs:
        if str(tx.get("to", "")).lower() != method["address"].lower():
            continue
        tx_hash = tx.get("hash", "")
        if not tx_hash:
            continue
        try:
            ...
            continue
        if abs(token_amount - expected_token_amount) > max(expected_token_amount * 0.0005, 1e-7):
            continue  # doesn't match this user's unique expected amount
        # Claim the hash atomically. Nothing after this point may `await` and risk
        # failing — see the comment in verify_ton_native_deposit for why. Credit the
        # USD amount already locked in at request time instead of re-fetching price.
        if not _claim_tx_hash(tx_hash):
            continue

        usd_amount = round(expected_usd, 3) if expected_usd is not None else round(token_amount, 3)
        _finalize_deposit(user_id, usd_amount, tx_hash, f"{method['label']} Autopay", token_amount)
        return usd_amount, token_amount, tx_hash
    return None
...
async def verify_deposit(method_key: str, user_id: int, expected_amount: float, memo_code: str = None, expected_usd: float = None):
    """Dispatches verification to the right chain handler based on the chosen payment method."""
    method = PAYMENT_METHODS[method_key]
    if method_key == "ton_gram":
        return await verify_ton_native_deposit(user_id, min_amount=expected_amount, memo_code=memo_code, expected_usd=expected_usd)
    if method_key == "ton_usdt":
        return await verify_ton_usdt_deposit(user_id, min_amount=expected_amount, memo_code=memo_code)
    return await verify_evm_deposit(method_key, user_id, expected_amount, expected_usd=expected_usd)
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "services":
        ...
        if result:
            ...
            text = (
                "✅ *Deposit Confirmed!*\n\n"
                f"💎 *Received:* `{token_amount:.6f} {method['symbol']}`\n"
            ...
            if method["uses_memo"]:
                checklist = (
                    ...
                    f"2️⃣ Sent the exact amount requested (`{pending['token']:.6f} {method['symbol']}`)\n"
                    f"3️⃣ Included the memo `{pending.get('code')}` exactly"
                )
            else:
                checklist = (
                    "1️⃣ Sent to the correct address\n"
                    f"2️⃣ Sent the *exact* amount shown — `{pending['token']:.6f} {method['symbol']}` "
...
async def deposit_amount_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if method["uses_memo"]:
        ...
        offset = generate_unique_evm_offset()
    token_needed = base_amount + offset
    ...
    if method["uses_memo"]:
        text = (
            ...
            f"💎 *Send exactly:* `{token_needed:.6f} {method['symbol']}`\n\n"
            "📥 *Address:*\n"
            f"`{method['address']}`\n\n"
        ...
        text = (
            ...
            "📥 *Address:*\n"
            f"`{method['address']}`\n\n"
            "⚠️ This network has *no memo field* — instead, send the *exact amount* above "
            "(every decimal matters). This amount is unique to *this* deposit only and can "
            "never be reused, so it can only ever match this one transaction.\n\n"
            "✅ After sending, tap *Verify Payment* below."
        )
    kb = InlineKeyboardMarkup([
